...
Si on sait que la fonction est strictement convexe (resp. concave) ---~
 ...
 un minimum (resp. maximum) unique.
...
\end{center}
\caption {Graphe de la fonction : $f(x) = (x-3)^2$}
\end{figure}
...
Notons que :
\begin{displaymath}
P(1|x) = \frac{exp(a+bx)}{1+exp(a+bx)}
\end{displaymath}
\begin{eqnarray}
P(0|x) &=& 1 - \frac{exp(a+bx)}{1+exp(a+bx)}\\
               &=&     \frac{1+ exp(a+bx) - exp(a+bx)}{1+exp(a+bx)}\\
                &=&  \frac{1}{1+exp(a+bx)}
\end{eqnarray}
Donc :
\begin{displaymath}
P(y |x) = \frac{exp(y(a+bx))}{1+exp(a+bx)}
\end{displaymath}
...
$exp(x)$ peut créer un overflow si $x$ trop grand.
...
négatif permet d'éviter l'underflow.
...
{\bf Dire de faire attention au problème d'explosion du score lors de
  la prédicion : exp(trop grand nombre)}
...
%Soit $\hat{\mathbf{y}} =  
%\mathop{\text{argmax}}_{y\in Y} \mathbf{w}^T \,
%    \boldsymbol\Phi(\mathbf{x},y)$

Soit $y^* = 
\mathop{\text{argmax}}_{y'\in Y\backslash\{y\}} \mathbf{w}^T \,
    \boldsymbol\Phi(\mathbf{x},y')$ (meilleur concurrent)
