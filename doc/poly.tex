...
\mathbf{left-arc} &\langle \mathbf{S}|w_i | w_j , \mathbf{B} , A   \rangle
&\Rightarrow& \langle \mathbf{S}| w_j , \mathbf{B} , A \cup \{
j\rightarrow i \}   \rangle\\
...
\begin{algorithm}
\begin{algorithmic}[0]
\Function{ArcStandardStaticOracle}{$\mathbf{S},\mathbf{B},A$}
\If{ $\mathbf{S} | i | j $} \Comment{Il y a au moins 2 éléments sur la
pile}
\If{$i\rightarrow j \in A$}
\State \Return $\mathbf{right-arc}$
\ElsIf{$j\rightarrow i \in A$}
