...
\item {\bf Récurrence:} Si $e \in AE(v)$ et que $|e| = k > 0$ alors $w(D(v)) = \times(s(e), w(D_1(v_1)), \ldots , w(D_k(v_k)))$.
\end{itemize}
\end{definition}
où $\times$ est une fonction qui combine les scores des $D_i$ dérivations qui mènent à $e$ avec le score $s(e)$ de $e$
...
\mathop{\text{max}}_{D \in {\cal D}(v)} w(D)&\text{sinon}
\end{array}
\end{displaymath}
\end{definition}
...
\State $\delta(v) \gets \text{max} (\delta(v) , \times(s(e),\delta(v_1) ,\ldots \delta (v_k)))$
\EndFor
\EndFor
\EndFunction
\end{algorithmic}
