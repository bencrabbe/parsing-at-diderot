...
\paragraph{Moyenne mobile cumulée (move to perceptron)} En pratique, plutôt que de stocker un grand
nombre de vecteurs de poids $\mathbf{w}_1\ldots \mathbf{w}_N$ pour
finalement calculer la moyenne $\bar{\mathbf{w}} = \frac{1}{N}
(\mathbf{w}_1+\ldots + \mathbf{w}_N$ en fin de procédure, on utilisera plutôt la méthode
de la moyenne mobile cumulée qui permet de mettre à jour la valeur de
$\bar{\mathbf{w}}$ itérativement~:
\begin{equation}
\label{eq-moving-average}
\bar{\mathbf{w}}_{i+1} = \frac{\mathbf{w}_{i+1} + i \bar{\mathbf{w}}_{i} }{i+1}
\end{equation}
On justifie cette méthode en voyant que~:
\begin{displaymath}
i \bar{\mathbf{w}}_{i} = \frac{i}{i} (\mathbf{w}_1 +\ldots +\mathbf{w}_i)\\
\end{displaymath}
par conséquent (\ref{eq-moving-average}) abrège~:
\begin{displaymath}
\bar{\mathbf{w}}_{i+1} = \frac{\mathbf{w}_{i+1} + \mathbf{w}_i +\ldots +\mathbf{w}_1 }{i+1}
\end{displaymath}
...
[ \text{score}(y_i) - (\text{score}(y^*_i) +1 ) ]\leq 0\\
0&\text{sinon}
\end{array}\right.
\end{equation}
...
\sum_{i=1}^N \left\{
\begin{array}{ll}
\phi_j(\mathbf{x}_{i},\hat{y}_i) -\phi_j(\mathbf{x}_{i},y_i) &\text{si } y_i
\not = \hat{y}_i\\
0&\text{sinon}
\end{array}\right.
\end{equation}
...
\left\{
\begin{array}{ll}
\boldsymbol\Phi(\mathbf{x}_i,\hat{y}_i)-\boldsymbol\Phi(\mathbf{x}_{i},y_i)
&\text{si } y_i \not = \hat{y}_i \\
 \mathbf{0}& \text{sinon}
\end{array}\right.
\end{displaymath}
...
\paragraph{Large marge et perceptron}

On peut voir l'algorithme du perceptron comme un cas particulier des
modèles à large marge. En posant que la marge est nulle, l'objectif à
large marge devient~:
\begin{equation}
f(\mathbf{w}) = \frac{1}{N}\sum_{i=1}^N \text{max}(0,
\mathbf{w}^T\boldsymbol\Phi(\mathbf{x}_i,y^*_i) - \mathbf{w}^T\boldsymbol\Phi(\mathbf{x}_i,y_i) ))
\end{equation}
dont le sous-gradient devient~:
\begin{equation}
\frac{\partial f(\mathbf{w})}{\partial w_j} =  
\sum_{i=1}^N \left\{
\begin{array}{ll}
...
\end{array}\right.
\end{equation}
Si l'algorithme utilisé pour estimer les paramètres est \ac{sgd}
(Algorithme ??), on peut observer que la méthode correspond exactement
à l'algorithme du \kw{ perceptron multiclasse}.

On peut donc voir l'algorithme du perceptron comme un cas particulier
des modèles à large marge.
...
stochastique moyennée présentés dans ce chapitre.
...
\While{non convergence}
\State
\scalebox{0.8}{$\mathbf{w} \gets \mathbf{w} - \alpha \left( \lambda \mathbf{w} + 
\frac{1}{N}\sum_{i=1}^N
\left\{
\begin{array}{ll}
...
\EndFunction
\end{algorithmic}
\caption{Algorithme de descente de gradient pour \ac{svm} linéaire (batch)}
\end{algorithm}
